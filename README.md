...
- **할 일 관리** — 프로젝트(색상 분류) · 작업 · 하위 작업(체크리스트)
  - 예상 뽀모도로, 우선순위, 마감일, 메모, **반복(매일/평일/매주 습관)** — 완료 시 다음 일정 자동 생성
  - 오늘 / 예정 / 언젠가 / 완료 그룹핑, 작업에서 바로 집중 시작
- **통계 & 리포트** — 오늘/주/월/년, 집중 시간·완료 뽀모도로·완료한 일·연속일(streak)
  - 시간대/요일/일/월별 막대 그래프, 프로젝트별 집중 분포
  - **전체 누적 집중 시간**, **집중 잔디(히트맵)**, **세션 기록**(언제 몇 분 집중했는지)
- **사운드** — 백색·핑크·브라운 노이즈, 빗소리·파도·모닥불·**새소리·시냇물·카페** (모두 **Web Audio로 실시간 합성**, 오디오 파일 0개)
  - **여러 소리 믹스**, **사운드 타이머**(N분 후 페이드아웃), 초침 소리, 종료 알림음(벨/차임/디지털/핑)
- **백업 / 복원** — 전체 데이터를 JSON으로 내보내기·가져오기 (로그인·클라우드 없이 로컬 보관)
- **다크 모드** — 라이트 / 다크 / 시스템 자동
- **다국어** — 한국어 / English (시스템 언어 자동 감지, 설정에서 즉시 전환)
